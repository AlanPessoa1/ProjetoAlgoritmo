...
public class TelaJogadorController {
    ...
    @FXML
    public void mostrarResolucaoLabirinto(ActionEvent event) {
        ...
        limparResolucaoAnterior(labirinto);
    ...
    @FXML
    public void irTelaResolucaoOriginal(ActionEvent event) throws IOException {
    ...
    private void semSaida() {
        Alert alert = new Alert(Alert.AlertType.WARNING);
        alert.setTitle("Aviso");
        alert.setHeaderText("Labirinto não possui caminho para a saída!");
        alert.showAndWait();
    }

    private void limparResolucaoAnterior(Labirinto labirinto) {
        int[][] matrizLabirinto = labirinto.obterLabirinto();
        for (int y = 0; y < matrizLabirinto.length; y++) {
            for (int x = 0; x < matrizLabirinto[y].length; x++) {
                ImageView imageView = (ImageView) gridLabirintoEditado.getChildren().get(y * labirinto.obterLargura() + x);
                if (matrizLabirinto[y][x] == 0) {
